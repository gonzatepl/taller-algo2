...
TEST(conjunto_test, test_stress) {
    ...
    for (int i = 0; i < NCLAVES; i++) {
	    ...
	    if (i % 2 == 0) {
            ...
            //AGREGADO POR MI: EL CARDINAL DISMINUYE DE A UNO POR VEZ,SI SE ELIMINA UN ELEMENTO DEL ARBOL
            ASSERT_EQ(c.cardinal(), NCLAVES - 1 - i / 2);

            //AGREGADO POR MI:
            //CADA VEZ QUE SE BORRA UN ELEMENTO PAR, TODOS LOS ANTERIORES IMPARES SIGUEN PERTENECIENDO AL ARBOL
            for (int j = 0; j<i; j++){
                int kk = clave(j);
                if (j % 2 == 1){
                    ASSERT_TRUE(c.pertenece(kk));
                }
            }
	    }
    ...
    for (int i = 0; i < NCLAVES; i++) { //ARRANCA A FALLAR CON EL ELEMENTO NRO 99, PIERDE MEMORIA, CON EL 101 YA SE CAE ENTERO
	    int k = clave(i);
	    if (i % 2 == 0) {
	    	ASSERT_FALSE(c.pertenece(k));
	    } else {
